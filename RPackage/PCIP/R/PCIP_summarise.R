...
#'
#' }
#' \item Read description table - BEST QUALITY (10, 01, 101, 1001) [[2]].
#' \itemize{
...
#' }
#'
#' \item Read description table - ALL READS [[3]].
#' \itemize{
#' \item readID (character). ID of the read.
#' \item ID (character). TARGET location into the HOST genome formated as chr:start-end.
#' \item strand (character). TARGET orientation related to the HOST genome.
...
PCIP_summarise <- function(PCIPbreakpoints = NULL, align = NULL, targetName = NULL, mergeISdistance = 200, mergeShearSiteDistance = 0, distanceFLAG = 1000, distance_report = 1000){
  ...
  breaks <- mutate(PCIPbreakpoints, index = 1:nrow(PCIPbreakpoints)) %>%
    ...
    filter(ligation %in% c("10", "01", "1001") & minDistanceEdge < 300 | ligation %in% c("101"))
  ...
  if(nrow(IS.table) > 0){
    ...
    LTR3.position <- if(is.null(IS.split[["edge3_breakPoint"]])){
      tibble(
        ...
        edge3.meanDistLTR = rep(NA, nrow(IS.split[["edge5_breakPoint"]])),
        edge3.sdDistLTR = rep(NA, nrow(IS.split[["edge5_breakPoint"]]))
      )
    } else {
  ...
  fastqID.best <- select(breaks, readID, groupIS) %>%
    left_join(
      breaks.summerised %>%
        # SELECT ONLY IS WITH 10 non-duplicated reads or for which ownership is certain
        filter(FLAG.READS == TRUE & FLAG.OWNER == TRUE) %>%
        rowwise() %>%
  ...
  # 6. When using 101 reads, DEL might be missed. To better retrieve them later on:

  # 6.1. Take the best IS (FLAG.OWNER == TRUE)
  bestIS <- select(breaks.summerised, -groupIS) %>%
    filter(FLAG.OWNER == TRUE)
  bestIS.gr <- GRanges(bestIS$ID)
  bestIS.gr$orientation <- bestIS$strand

  # 6.2. Take all the reads available
  align.all.gr <- align %>%
    # Exclude secundary alignments
    filter(alignmentType == "tp:A:P") %>%
    select(qName, tName, tStart, tEnd, orientationToRef) %>%
    # Exclude pure virus/pure host reads and reads mapping to multiple chromosomes
    ...
    ungroup() %>%
    # Keep only the host part of each reads and summarise it (start-end) %>%
    filter(tName != targetName) %>%
    group_by(qName) %>%
    mutate(start = min(tStart),
           end = max(tEnd),
           strand = paste0(unique(orientationToRef), collapse = ";")) %>%
    ungroup() %>%
    mutate(strand = ifelse(strand %in% c("+", "-"), strand, "*")) %>%
    select(-lig, -tStart, -tEnd) %>%
    makeGRangesFromDataFrame(seqnames.field = "tName", keep.extra.columns = T)

  # 6.3. Find the overlap between all available reads and the best IS
  overlaps <- findOverlaps(bestIS.gr, align.all.gr, maxgap = distance_report)

  # 6.4. Create a summary of the reads ID for each IS
  fastqID.all <- as_data_frame(align.all.gr)[subjectHits(overlaps),] %>%
    select(qName) %>%
    mutate(ID = paste(bestIS.gr[queryHits(overlaps)]),
           strand = bestIS.gr$orientation[queryHits(overlaps)]) %>%
    distinct()

  return(list(breaks.summerised, fastqID.best, fastqID.all))
