...
#' @param gapAlignment numeric or NA. Maximum mean gap tolerated between substrings. For debugging purpose. NA deactivate this option.
...
PCIP_getBreakPoints <- function(PAF = NULL, lengthTarget = NULL, targetName = NULL, gapAlignment = NA, distanceToLTR = NA, returnFILTEREDout = FALSE){
  ...
  if(!is.na(distanceToLTR)){
    print(paste0('CAREFUL: distanceToLTR will remove reads with gaps > ', distanceToLTR, ' near the LTR'))
